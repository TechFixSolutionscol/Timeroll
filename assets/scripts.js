document.addEventListener('DOMContentLoaded', () => {
    ...
    const appState = {
        clients: [], // Los clientes ahora se cargarán desde el backend
        timer: {
            interval: null,
            startTime: 0,
            elapsedTime: 0,
    ...
    // Carga los clientes desde el backend de Google Apps Script
    function loadClients() {
        google.script.run
            .withSuccessHandler(clients => {
                if (clients.error) {
                    alert('Error al cargar clientes: ' + clients.error);
                    return;
                }
    ...
    function generateEmailDraft() {
        ...
        google.script.run
            .withSuccessHandler(draft => {
                if (draft.error) {
                    elements.emailDraftDisplay.textContent = 'Error: ' + draft.error;
                } else {
            ...
            .withFailureHandler(err => {
                elements.emailDraftDisplay.textContent = 'Error de conexión al generar el borrador.';
                console.error(err);
                elements.generateEmailButton.disabled = false;
                elements.generateEmailSpinner.classList.add('hidden');
            })
            .generateEmailDraftBackend(prompt);
    }
    ...
    elements.clientForm.addEventListener('submit', (e) => {
        ...
        if (id) {
            ...
            google.script.run.withSuccessHandler(successCallback).withFailureHandler(failureCallback).updateClient(clientData);
        } else {
            google.script.run.withSuccessHandler(successCallback).withFailureHandler(failureCallback).addClient(clientData);
        }
    });
