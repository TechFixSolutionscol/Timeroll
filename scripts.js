document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleAuthSuccess(user) {
        ...
        elements.userInfo.textContent = `Usuario: ${user.name}`;
    ...
    function fetchClients() {
        google.script.run
            ...
            .withFailureHandler(err => showToast(`Error al obtener clientes: ${err.message}`))
            .getClients();
    }
    ...
    elements.signinButton.addEventListener('click', () => {
        ...
        if (!email || !password) {
            showToast('Por favor, introduce el correo y la contraseña.');
            return;
        }
        google.script.run
    ...
    elements.signupButton.addEventListener('click', () => {
        ...
        if (!name || !email || !password) {
            showToast('Por favor, rellena todos los campos para registrarte.');
            return;
        }
        google.script.run
            .withSuccessHandler(handleAuthSuccess)
    ...
    elements.clientForm.addEventListener('submit', (e) => {
        ...
        const onFailure = (err) => showToast(`Error: ${err.message}`);
