document.addEventListener('DOMContentLoaded', () => {
    ...
    async function generateEmailDraft() {
        ...
        try {
            ...
            if (result.candidates && result.candidates.length > 0 &&
              ...
              elements.emailDraftDisplay.textContent = text;
            } else {
              elements.emailDraftDisplay.textContent = 'No se pudo generar el borrador de correo. Inténtalo de nuevo.';
            }
        } catch (error) {
            console.error('Error al llamar a la API de Gemini:', error);
            elements.emailDraftDisplay.textContent = 'Error al generar el borrador. Por favor, revisa tu conexión.';
        } finally {
    ...
    elements.setupButton.addEventListener('click', async () => {
        if (!confirm('Esto configurará las tablas necesarias en tu Google Sheet. ¿Continuar?')) return;

        showToast('Configurando tablas... por favor espera.');
        const result = await callAppsScript({ action: 'setup' });

        if (result.status === 'success') {
            showToast(result.data); // Show success message from backend
        } else {
    ...
    elements.loginForm.addEventListener('submit', async (e) => {
        ...
        if (result.status === 'success' && result.data.success) {
            ...
            showToast(result.data.message || 'Error de inicio de sesión.');
        }
    });
    ...
    elements.clientForm.addEventListener('submit', async (e) => {
        ...
        if (id) {
            showToast('Actualizando cliente...');
            const result = await callAppsScript({ action: 'updateClient', data: { ...clientData, id } });
            if (result.status === 'success') {
                const index = appState.clients.findIndex(c => c.id === id);
                appState.clients[index] = result.data;
                showToast('Cliente actualizado con éxito.');
            } else {
                showToast(`Error: ${result.message}`);
            }
        } else {
            showToast('Añadiendo cliente...');
            const result = await callAppsScript({ action: 'addClient', data: clientData });
            if (result.status === 'success') {
                appState.clients.push(result.data);
    ...
    elements.clientsTableBody.addEventListener('click', async (e) => {
        ...
        if(target.classList.contains('delete-client-btn')) {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar este cliente?')) {
                showToast('Eliminando cliente...');
                const result = await callAppsScript({ action: 'deleteClient', data: { id } });
                if (result.status === 'success') {
    ...
    elements.sendInvoiceButton.addEventListener('click', async () => {
        if (appState.currentInvoiceData) {
            ...
            const result = await callAppsScript({ action: 'addInvoice', data: appState.currentInvoiceData });
            if (result.status === 'success') {
                showToast('Factura guardada y enviada (simulación).');
            } else {
                showToast(`Error al guardar factura: ${result.message}`);
            }
        }
...
`Hola ${client.name},\n\nTe envío la cuenta de cobro por la sesión realizada.\n\nDuración: ${hours.toFixed(2)} horas\nTarifa: $${rate.toFixed(2)} / hora\nTOTAL: $${total.toFixed(2)}\n\n¡Gracias por tu confianza!`;
    ...
    function init() {
        ...
        // Don't load clients here, wait for login
        switchView('dashboard');
    }

    async function loadClients() {
        showToast('Cargando clientes...');
        const result = await callAppsScript({ action: 'getClients' });
        if (result.status === 'success') {
            appState.clients = result.data;
            renderClients();
        } else {
            showToast(`Error al cargar clientes: ${result.message}`);
        }
    }
...
/**
 * @description Sends a request to the Google Apps Script backend.
 * @param {Object} body - The request body, which must include an 'action' property.
