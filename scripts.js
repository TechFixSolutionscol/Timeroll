document.addEventListener('DOMContentLoaded', () => {
    ...
    const SCRIPT_URL = ""; // <--- IMPORTANT: SET YOUR GOOGLE APPS SCRIPT URL HERE
    ...
    async function generateEmailDraft() {
        ...
        try {
            const response = await fetch(`${SCRIPT_URL}?action=generateEmail`, {
                method: 'POST',
                body: JSON.stringify(appState.currentInvoiceData)
            });
            if (!response.ok) {
                throw new Error("Network response was not ok");
            }
    ...
    elements.clientForm.addEventListener('submit', async (e) => {
        ...
        try {
            if (id) {
                clientData.id = id;
                await fetch(`${SCRIPT_URL}?action=updateClient`, {
                    method: 'POST',
                    body: JSON.stringify(clientData)
                });
                showToast('Cliente actualizado con éxito.');
            } else {
                await fetch(`${SCRIPT_URL}?action=addClient`, {
                    method: 'POST',
                    body: JSON.stringify(clientData)
                });
    ...
    elements.clientsTableBody.addEventListener('click', async e => {
        ...
        if(target.classList.contains('delete-client-btn')) {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar este cliente?')) {
                try {
                    await fetch(`${SCRIPT_URL}?action=deleteClient`, {
                        method: 'POST',
    ...
    elements.sendInvoiceButton.addEventListener('click', async () => {
        try {
            await fetch(`${SCRIPT_URL}?action=saveInvoice`, {
                method: 'POST',
                body: JSON.stringify(appState.currentInvoiceData)
            });
            ...
            showToast("Error al guardar factura.", true);
        }
    });
...
`Hola ${client.name},\n\nTe envío la cuenta de cobro por la sesión realizada.\n\nDuración: ${hours.toFixed(2)} horas\nTarifa: $${rate.toFixed(2)} / hora\nTOTAL: $${total.toFixed(2)}\n\n¡Gracias por tu confianza!`;
    ...
    elements.setupButton.addEventListener('click', async () => {
        try {
            await fetch(`${SCRIPT_URL}?action=setup`, { method: 'POST' });
            showToast('Tablas configuradas.');
        } catch (error) {
            showToast("Error al configurar las tablas.", true);
        }
    });
