document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        setupSheetsButton: $('#setup-sheets-button'),
    ...
    function handleSetupSheets() {
        elements.setupStatus.textContent = 'Creando tablas...';
        elements.setupSheetsButton.disabled = true;
        google.script.run.withSuccessHandler(response => {
            if(response.success){
                elements.setupStatus.textContent = response.message;
                elements.setupStatus.classList.add('text-green-500');
            } else {
                elements.setupStatus.textContent = 'Error: ' + response.message;
                elements.setupStatus.classList.add('text-red-500');
            }
            elements.setupSheetsButton.disabled = false;
        }).withFailureHandler(error => {
    ...
    elements.setupSheetsButton.addEventListener('click', handleSetupSheets);
    ...
    elements.sendInvoiceButton.addEventListener('click', () => {
        ...
        google.script.run.withSuccessHandler(response => {
            if(response.success){
                ...
                showToast('Error al guardar factura: ' + response.message)
            }
