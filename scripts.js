document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderClients(clients) {
        appState.clients = clients; // Update local cache
    ...
    function fetchClients() {
        google.script.run
            ...
            .withFailureHandler(error => showToast(`Error fetching clients: ${error.message}`))
            .getClients();
    }
    ...
    function handleLoginSuccess(user) {
        if (user) {
            ...
            elements.usernameDisplay.textContent = `Usuario: ${user.Username}`;
    ...
    elements.loginForm.addEventListener('submit', (e) => {
        ...
        google.script.run
            ...
            .withFailureHandler(error => showToast(`Error de acceso: ${error.message}`))
    ...
    elements.clientForm.addEventListener('submit', (e) => {
        ...
        const handler = {
            ...
            onFailure: (err) => showToast(`Error: ${err.message}`),
        };

        if (clientData.ID) {
            google.script.run
                .withSuccessHandler(() => handler.onSuccess('Cliente actualizado con éxito.'))
                .withFailureHandler(handler.onFailure)
                .updateClient(clientData);
        } else {
            google.script.run
                .withSuccessHandler(() => handler.onSuccess('Cliente añadido con éxito.'))
                .withFailureHandler(handler.onFailure)
    ...
    elements.setupButton.addEventListener('click', () => {
        google.script.run
            ...
            .withFailureHandler(error => showToast(`Setup failed: ${error.message}`))
            .setup();
    });
