...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function stopTimer() {
        ...
        await apiCall('addInvoice', invoiceData);
    ...
    async function generateEmailDraft() {
        ...
        const draft = await apiCall('generateEmailDraft', appState.currentInvoiceData);
        if (draft) {
            elements.emailDraftDisplay.textContent = draft;
        } else {
            elements.emailDraftDisplay.textContent = 'No se pudo generar el borrador. Inténtalo de nuevo.';
        }
    ...
    elements.loginButton.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            if (!SCRIPT_URL) {
                showToast("Por favor, configura la SCRIPT_URL en scripts.js");
                return;
            }
            loadClients();
        }, 300);
    ...
    elements.setupTablesButton.addEventListener('click', async () => {
        ...
        if (result) {
            showToast('Tablas configuradas con éxito.');
        }
    });
    ...
    elements.clientForm.addEventListener('submit', async (e) => {
        ...
        if (id) {
            const updatedClient = await apiCall('updateClient', clientData);
            if(updatedClient) {
                const index = appState.clients.findIndex(c => c.id === id);
                appState.clients[index] = updatedClient;
                showToast('Cliente actualizado con éxito.');
            }
        } else {
            const newClient = await apiCall('addClient', clientData);
            if(newClient) {
    ...
    elements.clientsTableBody.addEventListener('click', async e => {
        ...
        if(target.classList.contains('delete-client-btn')) {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar este cliente?')) {
                 const result = await apiCall('deleteClient', { id });
                 if(result) {
