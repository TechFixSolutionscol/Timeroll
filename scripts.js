document.addEventListener('DOMContentLoaded', () => {
    ...
    function saveClients() {
        ...
        localStorage.setItem('nextClientId', appState.nextClientId);
    }
    ...
    async function generateEmailDraft() {
        ...
        // **ADVERTENCIA DE SEGURIDAD:** Almacenar claves de API en el lado del cliente (como en localStorage)
        // no es seguro para aplicaciones en producción. Un usuario malintencionado podría encontrar
        // la clave fácilmente. Para un proyecto real, la llamada a la API de Gemini debería
        // realizarse desde un backend (servidor) seguro que gestione la clave.
        ...
        if (!apiKey) {
            ...
            if (apiKey) {
                ...
                elements.emailDraftDisplay.textContent = 'No se proporcionó una API Key. La generación de correo fue cancelada.';
...
`Hola ${client.name},\n\nTe envío la cuenta de cobro por la sesión realizada.\n\nDuración: ${hours.toFixed(2)} horas\nTarifa: $${rate.toFixed(2)} / hora\nTOTAL: $${total.toFixed(2)}\n\n¡Gracias por tu confianza!`;
    ...
    function init() {
        ...
        const savedNextClientId = localStorage.getItem('nextClientId');
        if (savedNextClientId) {
            appState.nextClientId = parseInt(savedNextClientId);
        }
