document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadClients() {
        google.script.run
            ...
            .withFailureHandler(error => {
                showToast(`Error loading clients: ${error.message}`);
            })
            .getClients();
    }
    ...
    elements.loginForm.addEventListener('submit', (e) => {
        ...
        elements.loginSubmitButton.textContent = 'Ingresando...';
        ...
        google.script.run
            .withSuccessHandler(response => {
                ...
                elements.loginSubmitButton.textContent = 'Ingresar';
            })
            .withFailureHandler(error => {
    ...
    elements.clientForm.addEventListener('submit', (e) => {
        ...
        const action = id ? 'updateClient' : 'addClient';
        google.script.run
            .withSuccessHandler(response => {
                if (response.success) {
...
`Hola ${client.name},\n\nTe envío la cuenta de cobro por la sesión realizada.\n\nDuración: ${hours.toFixed(2)} horas\nTarifa: $${rate.toFixed(2)} / hora\nTOTAL: $${total.toFixed(2)}\n\n¡Gracias por tu confianza!`;
    ...
    elements.setupButton.addEventListener('click', () => {
        ...
        elements.setupButton.textContent = 'Creando...';
        google.script.run
            .withSuccessHandler(response => {
                showToast(response);
                elements.setupButton.disabled = false;
                elements.setupButton.textContent = 'Crear Tablas en Google Sheets';
            })
            .withFailureHandler(error => {
                showToast(`Error: ${error.message}`);
                elements.setupButton.disabled = false;
