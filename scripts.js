document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadClients() {
        google.script.run
            ...
            .withFailureHandler(err => {
                showToast('Error al cargar clientes: ' + err.message);
            })
            .getClients();
    }
    ...
    function stopTimer() {
        ...
        if (client) {
          toggleInvoiceModal(true, { client, hours, rate, total });
            ...
            showToast('Error: No se pudo encontrar el cliente seleccionado.');
        }
    ...
    function generateEmailDraft() {
        if (!appState.currentInvoiceData) {
            showToast('No hay datos de factura para generar el correo.');
            return;
        }
        ...
        google.script.run
            .withSuccessHandler(response => {
                if (response.text) {
                    elements.emailDraftDisplay.textContent = response.text;
                } else {
                    elements.emailDraftDisplay.textContent = 'Error: ' + response.error || 'No se pudo generar el borrador.';
                }
                elements.generateEmailButton.disabled = false;
                elements.generateEmailSpinner.classList.add('hidden');
            })
            .withFailureHandler(err => {
                elements.emailDraftDisplay.textContent = 'Error al generar el borrador: ' + err.message;
                elements.generateEmailButton.disabled = false;
                elements.generateEmailSpinner.classList.add('hidden');
            })
            .generateEmailDraftSecure(prompt);
    }
    ...
    function setupEventListeners() {
        ...
        elements.clientForm.addEventListener('submit', (e) => {
            e.preventDefault();
            const id = parseInt($('#client-id').value);
            const clientData = {
            ...
            const handler = (savedClient) => {
                if (savedClient && !savedClient.error) {
                    if (id) { // Update
                        ...
                        appState.clients.push(savedClient);
                        showToast('Cliente añadido con éxito.');
                    }
                    renderClients();
                    toggleClientModal(false);
                } else {
                    showToast('Error al guardar cliente: ' + (savedClient ? savedClient.error : ''));
                }
            };

            const errorHandler = err => showToast('Error: ' + err.message);

            if (id) {
                google.script.run.withSuccessHandler(handler).withFailureHandler(errorHandler).updateClient(clientData);
            } else {
                google.script.run.withSuccessHandler(handler).withFailureHandler(errorHandler).addClient(clientData);
            }
        });
                    ...
                    google.script.run
                        .withSuccessHandler(response => {
                            if (response.success) {
                                ...
                                showToast('Error al eliminar: ' + response.error);
                            }
                        })
