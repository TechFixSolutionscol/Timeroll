document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderClients(clients) {
        appState.clients = clients; // Update local state
    ...
    function loadClients() {
        google.script.run
            ...
            .withFailureHandler(err => {
                ...
                console.error("Error fetching clients from backend: ", err);
            })
            .getClients();
    }
    ...
    function toggleClientModal(show = false, client = null) {
        if (show) {
            ...
            if (client) {
                ...
                $('#client-id').value = ''; // Ensure ID is cleared for new clients
            }
    ...
    function generateEmailDraft() {
        ...
        google.script.run
            ...
            .generateEmailDraft(hours, total, client.name, client.email);
    }
    ...
    elements.clientForm.addEventListener('submit', (e) => {
        ...
        const onSuccess = (updatedClients) => {
            showToast(`Cliente ${id ? 'actualizado' : 'añadido'} con éxito.`);
            loadClients(); // Reload all clients from the source of truth
            toggleClientModal(false);
        };

        const onFailure = (err) => {
            showToast(`Error al guardar cliente: ${err.message}`);
        ...
        if(target.classList.contains('delete-client-btn')) {
            ...
            if (confirm('¿Estás seguro de que quieres eliminar este cliente?')) {
                google.script.run
                    ...
                    .withFailureHandler((err) => {
                         showToast(`Error al eliminar cliente: ${err.message}`);
    ...
    elements.sendWhatsappButton.addEventListener('click', () => {
        ...
        if (numero && !numero.startsWith('57') && numero.length === 10) { // Common case for Colombia
            numero = '57' + numero;
        }
